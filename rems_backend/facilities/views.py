...
class BookingViewSet(ModelViewSet):
    ...
    def clearance(self, request, pk=None):
        ...
        now = timezone.now()
        previous_clearance = booking.security_clearance_completed
        ...
        with transaction.atomic():
            ...
            # REAL AUDIT EVENT
            # ----------------------------------------------------
            # The audit event is created only after the booking
            # state has been successfully changed. Because the
            # entire operation is inside transaction.atomic(),
            # a failure to write the audit event also prevents
            # the booking change from being committed.
            AuditService.success(
                module=AuditEvent.Module.FACILITIES,
                action=AuditEvent.Action.CLEARANCE,
                event_type=(
                    "FACILITY_SECURITY_CLEARANCE_COMPLETED"
                ),
                description=(
                    f"Security clearance completed for "
                    f"facility booking #{booking.id}. "
                    f"The booking guest list is now locked."
                ),
                user=request.user,
                request=request,
                object_type="Booking",
